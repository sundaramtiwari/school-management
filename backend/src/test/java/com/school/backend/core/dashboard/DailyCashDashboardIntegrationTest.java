...
public class DailyCashDashboardIntegrationTest extends BaseAuthenticatedIntegrationTest {
    ...
    void daily_cash_dashboard_should_include_only_cash_mode_data() {
        ...
        ResponseEntity<Map> classResp = restTemplate.exchange(
                "/api/classes",
                HttpMethod.POST,
                new HttpEntity<>(Map.of("name", "8", "sessionId", sessionId, "schoolId", schoolId), headers),
                Map.class);
        ...
        Long feeTypeId = Objects.requireNonNull(restTemplate
                .exchange("/api/fees/types", HttpMethod.POST, new HttpEntity<>(feeType, headers), FeeType.class)
                .getBody()).getId();
        ...
        cashPayment.setAmountPaid(new BigDecimal("500.00"));
        cashPayment.setMode("CASH");
        cashPayment.setPaymentDate(date);
        restTemplate.exchange("/api/fees/payments", HttpMethod.POST, new HttpEntity<>(cashPayment, headers), Object.class);
        ...
        upiPayment.setAmountPaid(new BigDecimal("100.00"));
        upiPayment.setMode("UPI");
        upiPayment.setPaymentDate(date);
        restTemplate.exchange("/api/fees/payments", HttpMethod.POST, new HttpEntity<>(upiPayment, headers), Object.class);
        ...
        restTemplate.exchange("/api/expenses", HttpMethod.POST, new HttpEntity<>(cashExpense, headers), Object.class);
        ...
        restTemplate.exchange("/api/expenses", HttpMethod.POST, new HttpEntity<>(upiExpense, headers), Object.class);
