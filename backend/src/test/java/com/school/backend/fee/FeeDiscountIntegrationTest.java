...
public class FeeDiscountIntegrationTest extends BaseAuthenticatedIntegrationTest {
        ...
        void applyDiscount_should_create_adjustment_and_update_assignment_summary() {
                ...
                ResponseEntity<FeeDiscountApplyResponse> response = restTemplate.exchange(
                                "/api/fees/assignments/" + assignmentId + "/discount",
                                HttpMethod.POST,
                                new HttpEntity<>(req, headers),
                                FeeDiscountApplyResponse.class);

                Assertions.assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
                FeeDiscountApplyResponse applyResp = Objects.requireNonNull(response.getBody());
                Assertions.assertThat(applyResp.getAppliedAmount()).isEqualByComparingTo(new BigDecimal("100.00"));

                StudentFeeAssignmentDto dto = restTemplate.exchange(
                                "/api/fees/assignments/students/" + studentId,
                                HttpMethod.GET,
                                new HttpEntity<>(headers),
                                StudentFeeAssignmentDto[].class).getBody()[0];
                ...
                Assertions.assertThat(adjustment.getDiscountValueSnapshot())
        ...
        void applyDiscount_should_reject_when_discount_exceeds_remaining_principal() {
                ...
                ResponseEntity<FeeDiscountApplyResponse> firstResp = restTemplate.exchange(
                                "/api/fees/assignments/" + assignmentId + "/discount",
                                HttpMethod.POST,
                ...
                ResponseEntity<FeeDiscountApplyResponse> secondResp = restTemplate.exchange(
                                "/api/fees/assignments/" + assignmentId + "/discount",
                                HttpMethod.POST,
                                new HttpEntity<>(secondReq, headers),
                                FeeDiscountApplyResponse.class);

                Assertions.assertThat(secondResp.getStatusCode()).isEqualTo(HttpStatus.OK);
                Assertions.assertThat(secondResp.getBody().isCapped()).isTrue();
                Assertions.assertThat(secondResp.getBody().getAppliedAmount())
                                .isEqualByComparingTo(new BigDecimal("200.00"));

                BigDecimal currentDiscount = assignmentRepository.findById(assignmentId)
                                .orElseThrow()
                                .getTotalDiscountAmount();
                Assertions.assertThat(currentDiscount).isEqualByComparingTo(new BigDecimal("1000.00"));
                Assertions.assertThat(feeAdjustmentRepository.findByAssignmentId(assignmentId)).hasSize(2);
        }
        ...
        void adjustmentHistory_should_use_snapshot_and_fallback_legacy_discount() {
                ...
                ResponseEntity<FeeDiscountApplyResponse> applyResp = restTemplate.exchange(
                                "/api/fees/assignments/" + assignmentId + "/discount",
                                HttpMethod.POST,
                                new HttpEntity<>(req, headers),
                                FeeDiscountApplyResponse.class);
