...
public class FeePaymentAllocationIntegrationTest extends BaseAuthenticatedIntegrationTest {
        ...
        void payment_should_allocate_to_late_fee_first_then_principal() {
                ...
                assignReq.setDueDate(LocalDate.now().minusDays(10)); // Overdue (Flat 50 late fee)
                ...
                Long assignmentId = Objects.requireNonNull(assignResp.getBody()).getId();
                ...
                ResponseEntity<FeePaymentDto> payResp = restTemplate.exchange(
                                "/api/fees/payments", HttpMethod.POST, new HttpEntity<>(payReq, headers),
                                FeePaymentDto.class);
                ...
                Assertions.assertThat(allocations.get(0).getPrincipalAmount())
                                .isEqualByComparingTo(new BigDecimal("50.00"));
                Assertions.assertThat(allocations.get(0).getLateFeeAmount())
                                .isEqualByComparingTo(new BigDecimal("50.00"));

                // 5. Verify Final Dues
                summaryResp = restTemplate.exchange(summaryUrl, HttpMethod.GET, new HttpEntity<>(headers),
                                FeeSummaryDto.class);
        ...
        @Test
        void payment_should_reject_partial_late_fee_payment() {
                setupBaseData();
                StudentFeeAssignRequest assignReq = new StudentFeeAssignRequest();
                assignReq.setStudentId(studentId);
                assignReq.setFeeStructureId(feeStructureId);
                ...
                payReq.setAllocations(List.of(
                                FeePaymentAllocationRequest.builder()
                                                .assignmentId(assignmentId)
                                                .principalAmount(new BigDecimal("30.00")) // Less than 50 late fee
                                                .build()));
                payReq.setMode("CASH");

                ResponseEntity<Map> payResp = restTemplate.exchange(
                                "/api/fees/payments", HttpMethod.POST, new HttpEntity<>(payReq, headers), Map.class);

                Assertions.assertThat(payResp.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
                Assertions.assertThat(payResp.getBody().get("message").toString()).contains("Late fee");
        }

        @Test
        void payment_should_reject_overpayment() {
                setupBaseData();
                StudentFeeAssignRequest assignReq = new StudentFeeAssignRequest();
                assignReq.setStudentId(studentId);
                assignReq.setFeeStructureId(feeStructureId);
                assignReq.setSessionId(sessionId);
                assignReq.setDueDate(LocalDate.now()); // No late fee
                ...
                payReq.setAllocations(List.of(
                                FeePaymentAllocationRequest.builder()
                                                .assignmentId(assignmentId)
                                                .principalAmount(new BigDecimal("1001.00")) // Assignment is 1000
                                                .build()));
                payReq.setMode("CASH");
                ...
                Assertions.assertThat(payResp.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
                Assertions.assertThat(payResp.getBody().get("message").toString())
                                .contains("exceeds total outstanding");
        }

        @Test
        void payment_should_reject_duplicate_allocations() {
                setupBaseData();
                StudentFeeAssignRequest assignReq = new StudentFeeAssignRequest();
                assignReq.setStudentId(studentId);
                assignReq.setFeeStructureId(feeStructureId);
                ...
                payReq.setStudentId(studentId);
                payReq.setAllocations(List.of(
                                FeePaymentAllocationRequest.builder().assignmentId(assignmentId)
                                                .principalAmount(new BigDecimal("100")).build(),
                                FeePaymentAllocationRequest.builder().assignmentId(assignmentId)
                                                .principalAmount(new BigDecimal("100")).build()));
                payReq.setMode("CASH");
                ...
                Assertions.assertThat(payResp.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
                Assertions.assertThat(payResp.getBody().get("message").toString()).contains("Duplicate allocation");
        }
        ...
        void head_summary_should_return_fee_type_totals_for_date() {
                ...
                payReq.setAllocations(List.of(
                                FeePaymentAllocationRequest.builder()
                                                .assignmentId(assignmentId)
                                                .principalAmount(new BigDecimal("120.00")) // 50 late + 70 principal
                                                .build()));
                payReq.setMode("CASH");
                payReq.setPaymentDate(LocalDate.now());
        ...
        private void setupBaseData() {
                // Create School
                Map<String, Object> schoolReq = Map.of("name", "Allocation Test School", "displayName", "ATS", "board",
                                "CBSE",
                                "schoolCode", "ATS-001");
